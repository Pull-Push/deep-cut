...
export async function getDiscogsLabelRoster(labelId, page = 1) {
    try {
        const response = await fetch(`https://api.discogs.com/labels/${labelId}/releases?per_page=25&page=${page}&sort=year&sort_order=desc`,
            {
                headers:{
                    "User-Agent": "DeepCut/1.0 +https://github.com/pull-push/deep-cut",
                },
...
export async function getDiscogsRelease(releaseId) {
    try {
        ...
        if(!response.ok){
            console.error("Error getting discogs release", response.status)
            return null
        }
        const data = await response.json()
        return data
    } catch (error) {
        console.error('Failed to get Discogs release', error)
        return null
    }
}

export async function getDiscogsArtist(artistId) {
    try {
        ...
        if(!response.ok){
            console.error("Error getting discogs artist", response.status)
            return null
        }
        const data = await response.json()
        return data
    } catch (error) {
        console.error("Failed to get Discogs artist", error)
        return null
    }
}

export async function getDiscogsArtistReleases(artistId) {
    try {
        const response = await fetch(`https://api.discogs.com/artists/${artistId}/releases?per_page=20&sort=year&sort_order=desc`,
            {
                headers:{
                    "User-Agent": "DeepCut/1.0 +https://github.com/pull-push/deep-cut",
                },
        ...
        if(!response.ok){
            console.error("Error getting discogs artist releases", response.status)
            return null
        }
        const data = await response.json()
        return data
    } catch (error) {
        console.error("Failed to get Discogs artist releases" , error)
        return null
    }
}