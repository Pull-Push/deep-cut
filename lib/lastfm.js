...
export async function getSimilarTags(genre) {
    try{
        const encodedGenre = genre.trim().toLowerCase().replace(/\s+/g, '+');
        const response = await fetch(`https://ws.audioscrobbler.com/2.0/?method=tag.getsimilar&tag=${encodedGenre}&api_key=${LASTFM_TOKEN}&format=json`)
        const data = await response.json()

        // console.log('lastfm data', data.similartags.tag)
        if(data.error){
            console.error('getSimilarTags last.fm error', data.message)
            return null
        }
        return data
    } catch(error){
        console.error('Failed to get similar tags', error)
        return null
    }
}